		...
		for _, op := range ops {
			for _, isLHS := range []bool{false, true} {
				for _, tc := range testCases {
					...
					ok := DispatchBinaryTrailingAVX512(op, isLHS, unsafe.Pointer(&lhs[0]), unsafe.Pointer(&rhs[0]), unsafe.Pointer(&out[0]), A, B, dtypes.Float32)
					if !ok {
						t.Fatalf("DispatchBinaryTrailingAVX512 returned false for %s f32", op)
					}
		...
		for _, op := range ops {
			for _, isLHS := range []bool{false, true} {
				for _, tc := range testCases {
					...
					ok := DispatchBinaryTrailingAVX512(op, isLHS, unsafe.Pointer(&lhs[0]), unsafe.Pointer(&rhs[0]), unsafe.Pointer(&out[0]), A, B, dtypes.Float64)
					if !ok {
						t.Fatalf("DispatchBinaryTrailingAVX512 returned false for %s f64", op)
					}
		...
		for _, op := range intOps {
			for _, isLHS := range []bool{false, true} {
				for _, tc := range testCases {
					...
					ok := DispatchBinaryTrailingAVX512(op, isLHS, unsafe.Pointer(&lhs[0]), unsafe.Pointer(&rhs[0]), unsafe.Pointer(&out[0]), A, B, dtypes.Int32)
					if !ok {
						t.Fatalf("DispatchBinaryTrailingAVX512 returned false for %s int32", op)
					}
		...
		for _, op := range uintOps {
			for _, isLHS := range []bool{false, true} {
				for _, tc := range testCases {
					...
					ok := DispatchBinaryTrailingAVX512(op, isLHS, unsafe.Pointer(&lhs[0]), unsafe.Pointer(&rhs[0]), unsafe.Pointer(&out[0]), A, B, dtypes.Uint32)
					if !ok {
						t.Fatalf("DispatchBinaryTrailingAVX512 returned false for %s uint32", op)
					}
		...
		for _, op := range int64Ops {
			for _, isLHS := range []bool{false, true} {
				for _, tc := range testCases {
					...
					ok := DispatchBinaryTrailingAVX512(op, isLHS, unsafe.Pointer(&lhs[0]), unsafe.Pointer(&rhs[0]), unsafe.Pointer(&out[0]), A, B, dtypes.Int64)
					if !ok {
						t.Fatalf("DispatchBinaryTrailingAVX512 returned false for %s int64", op)
					}
