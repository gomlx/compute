...
func execSmallTransposedGeneric[T gobackend.PODNumericConstraints]( //alt:base
	//alt:half func execSmallTransposedHalfPrecision[T dtypes.HalfPrecision[T], P dtypes.HalfPrecisionPtr[T]](
	lhs, rhs, output *gobackend.Buffer, params *NodeData, batchStartIdx, batchEndIdx int) {
	lhsFlat := lhs.Flat.([]T)
	rhsFlat := rhs.Flat.([]T)
	outputFlat := output.Flat.([]T) //alt:base
	//alt:half outputFlat := output.Flat.([]float32)
					...
					for idxLhsCross := outerIdxLhsCross; idxLhsCross < lhsCrossBlockEnd; idxLhsCross++ {
						...
						for idxRhsCross := outerIdxRhsCross; idxRhsCross < rhsCrossBlockEnd; idxRhsCross++ {
							...
							for ; idxContracting+7 < contractingBlockEnd; idxContracting += 8 {
								...
								//alt:half sum += lhsFlat[lhsRowStartIdx+idxContracting].Float32()*rhsFlat[rhsColStartIdx+idxContracting].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+1].Float32()*rhsFlat[rhsColStartIdx+idxContracting+1].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+2].Float32()*rhsFlat[rhsColStartIdx+idxContracting+2].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+3].Float32()*rhsFlat[rhsColStartIdx+idxContracting+3].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+4].Float32()*rhsFlat[rhsColStartIdx+idxContracting+4].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+5].Float32()*rhsFlat[rhsColStartIdx+idxContracting+5].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+6].Float32()*rhsFlat[rhsColStartIdx+idxContracting+6].Float32() +
								//alt:half	lhsFlat[lhsRowStartIdx+idxContracting+7].Float32()*rhsFlat[rhsColStartIdx+idxContracting+7].Float32()
							}
