...
var (
	...
	DotGeneralTargetBlockLog2Dim [gobackend.MaxDTypes]int
...
func setDotGeneralTargetBlockSize(blockSize int) {
	...
	for _, dtype := range gobackend.NumericDTypes {
...
func blockForDotGeneral(f *gobackend.Function, input *gobackend.Node,
	contractingAxes, batchAxes []int,
	batchSize, axesASize, axesBSize int) *gobackend.Node {
	...
	blocked, _ := f.GetOrCreateNode(compute.OpTypeBlockForDotGeneral, blockedShape, []*gobackend.Node{input}, data)
	return blocked
}
...
func execBlockForDotGeneral(backend *gobackend.Backend, node *gobackend.Node, inputs []*gobackend.Buffer, _ []bool) (*gobackend.Buffer, error) {
	...
	copyFlatToBlock := copyFlatToBlockAny.(func(source, blkOutput *gobackend.Buffer, contractingAxes,
...
func execDotGeneralBlocked(backend *gobackend.Backend, lhsBlocks, rhsBlocks *gobackend.Buffer, hasBatch bool, params *NodeData,
	...
	kernelBuilder := kernelBuilderAny.(func(lhs, rhs, output *gobackend.Buffer, blockDim int) kernelFuncType)
