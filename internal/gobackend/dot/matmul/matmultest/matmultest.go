...
// Package matmultest provides test helpers for verifying matrix multiplication packing and output routines.
package matmultest
...
// RunPackLHSTests verifies that packLHSFn matches the reference matmul.PackLHS implementation.
func RunPackLHSTests[T gotype.NumericNotComplex](t *testing.T, packLHSFn PackLHSFn[T], lhsL1KernelRows int) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("LHS/%dx%d_at_%d_%d", tc.rows, tc.cols, tc.rowStart, tc.colStart), func(t *testing.T) {
			...
			matmul.PackLHS(src, dstExpected, tc.rowStart, tc.colStart, totalCols, tc.rows, tc.cols, lhsL1KernelRows)
...
// RunPackLHSTestsHalfPrecision verifies that packLHSFn matches the reference matmul.PackLHS for half-precision types.
...
// RunPackRHSTests verifies that packRHSFn matches the reference matmul.PackRHS implementation.
func RunPackRHSTests[T gotype.NumericNotComplex](t *testing.T, packRHSFn PackRHSFn[T], rhsL1KernelCols int) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("RHS/%dx%d_at_%d_%d", tc.rows, tc.cols, tc.rowStart, tc.colStart), func(t *testing.T) {
			...
			// Reference implementation
			matmul.PackRHS(src, want, tc.rowStart, tc.colStart, totalCols, tc.rows, tc.cols, rhsL1KernelCols)
			// Implementation under test
...
// RunPackRHSTestsHalfPrecision verifies that packRHSFn matches the reference matmul.PackRHS for half-precision types.
...
// RunApplyPackedOutputTests verifies that applyFn matches the reference matmul.ApplyPackedOutput.
func RunApplyPackedOutputTests[T gotype.NumericNotComplex](t *testing.T, applyFn ApplyPackedOutputFn[T]) {
	...
	t.Run("Apply/Overwrite", func(t *testing.T) {
		matmul.ApplyPackedOutput(packedOutput, outputNoSIMD, true, packedOutputRowStride, 0, 1, outputRowStride, height, width)
	...
	t.Run("Apply/Accumulate", func(t *testing.T) {
		matmul.ApplyPackedOutput(packedOutput, outputNoSIMD, false, packedOutputRowStride, 0, 1, outputRowStride, height, width)
...
// RunBenchmarkPackLHS runs a benchmark for a packLHS function.
