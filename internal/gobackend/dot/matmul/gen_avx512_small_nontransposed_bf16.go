...
func transposeRHSBFloat16(src, dst []bfloat16.BFloat16, K, N int) { //alt:bf16
...
func avx512SmallBFloat16NonTransposedAsm( //alt:bf16
	...
	if rhsCrossSize == 1 {
		...
		avx512SmallBFloat16TransposedAsm(lhs, rhs, batchStart, batchCount, lhsCrossSize, 1, contractingSize, output) //alt:bf16
	...
	if rhsCrossSize < vecWidth {
		...
		if kn <= len(stackRhsT) {
			...
			rhsT = make([]bfloat16.BFloat16, kn) //alt:bf16
		...
		for b := range batchCount {
			...
			transposeRHSBFloat16(rhsSlice, rhsT, contractingSize, rhsCrossSize) //alt:bf16
			...
			avx512SmallBFloat16TransposedAsm(lhsSlice, rhsT, 0, 1, lhsCrossSize, rhsCrossSize, contractingSize, outSlice) //alt:bf16
	...
	for range batchCount {
		...
		for ; row+3 < lhsCrossSize; row += 4 {
			...
			for ; col+63 < rhsCrossSize; col += 64 { //alt:f32|bf16|f16
				...
				avx512SmallNonTransposedTile4x64BFloat16Asm( //alt:bf16
			...
			for ; col+15 < rhsCrossSize; col += 16 { //alt:f32|bf16|f16
				...
				avx512SmallNonTransposedTile4x16BFloat16Asm( //alt:bf16
			...
			for ; col < rhsCrossSize; col++ {
				...
				for k := range contractingSize {
					...
					l0 := (*(*bfloat16.BFloat16)(unsafe.Pointer(uintptr(lRow0) + kOffset))).Float32() //alt:bf16
					...
					l1 := (*(*bfloat16.BFloat16)(unsafe.Pointer(uintptr(lRow1) + kOffset))).Float32() //alt:bf16
					...
					l2 := (*(*bfloat16.BFloat16)(unsafe.Pointer(uintptr(lRow2) + kOffset))).Float32() //alt:bf16
					...
					l3 := (*(*bfloat16.BFloat16)(unsafe.Pointer(uintptr(lRow3) + kOffset))).Float32() //alt:bf16
					...
					rVal := (*(*bfloat16.BFloat16)(unsafe.Pointer(rhsBase + uintptr(k*rhsCrossSize+col)*elemSize))).Float32() //alt:bf16
