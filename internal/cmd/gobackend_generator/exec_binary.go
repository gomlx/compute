...
{{- end}}
	...
	default:
		...
		{{- if $is_comparison }}
			output[indices.TgtFlatIdx] = {{CallOp .Format "a" "b"}}
		{{- else }}
			output[indices.TgtFlatIdx] = bfloat16.FromFloat32({{CallOp .Format "a" "b"}})
		{{- end }}
		}
	return
	}
...
func GenerateExecBinary() {
	...
	klog.V(1).Infof("\t%s\n", cmd)
	must.M(cmd.Run())
	fmt.Printf("✅ gobackend_generator:  \tsuccessfully generated %s\n", fileName)
}
